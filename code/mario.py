...
def loop_func(num):
    global gamePad, bgd
    bgd = pygame.image.load('howtomario.png')
    for i in range(num):                
        canvas.blit(bgd,(0,0))
...
class Dragon:
    ...
    def __init__(self):
        ...
        self.dragon_img_rect.width -= 20
        self.dragon_img_rect.height -= 20
        self.dragon_img_rect.top = WINDOW_HEIGHT/2-20
    ...
    def update(self):
        ...
        if self.mario_img_rect.bottom >= fire_img_rect.top:
            ...
            if SCORE > self.mario_score:
                self.mario_score = SCORE
        if self.up:
            self.mario_img_rect.top -= 25
        if self.down:
            self.mario_img_rect.bottom += 25
...
def check_level(SCORE):
    ...
    if SCORE in range(0, 25):
        cactus_img_rect.bottom = 75
        fire_img_rect.top = WINDOW_HEIGHT - 125
        LEVEL = 1
    elif SCORE in range(25, 50):
        cactus_img_rect.bottom = 125
        fire_img_rect.top = WINDOW_HEIGHT - 175
        LEVEL = 2
    elif SCORE in range(50, 75):
        cactus_img_rect.bottom = 175
        fire_img_rect.top = WINDOW_HEIGHT - 225
        LEVEL = 3
    elif SCORE > 75:
        cactus_img_rect.bottom = 225
        fire_img_rect.top = WINDOW_HEIGHT - 275
        LEVEL = 4
...
def game_loop():
    while True:
        ...
        while True:
            ...
            if (input1 == False):
                mario.up = True
                print_font = font.render('1 pressed', True, GREEN)
                print_font_rect = print_font.get_rect()
                print_font_rect.center = (200, mario.mario_img_rect.bottom)# + print_font_rect.height/2)
                canvas.blit(print_font, print_font_rect)
                mario.down = False
                
            if (input2 == False):
                mario.up = False
                mario.down = True
                print_font = font.render('2 pressed', True, GREEN)
                print_font_rect = print_font.get_rect()
                print_font_rect.center = (200, mario.mario_img_rect.bottom)# + print_font_rect.height/2)
                canvas.blit(print_font, print_font_rect)
