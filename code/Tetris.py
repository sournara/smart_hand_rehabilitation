...
channel = [input1,input2,input3,input4]
...
def loop_func(num):
    global canvas, bgd
    canvas = pygame.display.set_mode((s_width, s_height))
    bgd = pygame.image.load('howtotetris.jpg')
    for i in range(num):                
        drawObject(bgd, 0, 0)
        sleep(1)
...
gp.add_event_detect(input1, gp.FALLING, callback=switchPressed, bouncetime=200)
gp.add_event_detect(input2, gp.FALLING, callback=switchPressed, bouncetime=200)
gp.add_event_detect(input3, gp.FALLING, callback=switchPressed, bouncetime=200)
gp.add_event_detect(input4, gp.FALLING, callback=switchPressed, bouncetime=200)
...
col = 10  # 10 columns
row = 20  # 20 rows
s_width = 1920  # window width 800
s_height = 1080  # window height 750
play_width = 450  # play window width; 300/10 = 30 width per block
play_height = 900  # play window height; 600/20 = 20 height per block
block_size = 45  # size of block

top_left_x = (s_width - play_width) // 2 # 250 -> 735
top_left_y = s_height - play_height - 50 # 100 -> 130
...
def main_menu(window):
    ...
    while run:
        ...
        input3=gp.input(13)
        input4=gp.input(19)
        if (input1 == False) or (input2 == False) or (input3 == False) or (input4 == False) :
            main(window)
